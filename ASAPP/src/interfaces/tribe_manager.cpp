...
{
    using components::tribelog_message;
    ...
    namespace
    {
        ...
        tribelog_message::Timestamp parse_timestamp(const cv::Mat& src)
        {
            ...
            return tribelog_message::Timestamp::parse(util::fix(raw, OCR_FIXES));
        }
    ...
    void tribe_manager::update_tribelogs(const LogUpdateCallback& on_finish,
                                        const std::chrono::seconds receive_for)
    {
        ...
        std::thread([this, on_finish, logs]() -> void {
            ...
            for (const auto& entry : entries) {
                const tribelog_message msg = parse(cv::Mat(logs, entry.to_cv()));
    ...
    std::vector<window::Rect> tribe_manager::collect_entries(const cv::Mat& src) const
    {
    ...
    void tribe_manager::add_message(const components::tribelog_message& msg)
    {
