...
namespace asa::settings
{
	...
	bool open_file(
		bool verbose, std::filesystem::path path, std::ifstream& out_file)
	{
		if (!std::filesystem::exists(path)) {
			std::cout << std::format(
				"[!] Path '{}' was not found.", path.string());
			return false;
		}
	...
	static bool parse_key_value(
		std::string token, std::string& key_out, std::string& value_out)
	{
		...
		if (key_out == "ActionName") {
			value_out = token.substr(
				eq + 2, token.length() - (key_out.length() + 3));
		}
		// Parse out the ending parantheses, example: Key=F)
		else if (key_out == "Key") {
			value_out = token.substr(
				eq + 1, token.length() - key_out.length() - 2);
		}
		else {
			value_out = token.substr(eq + 1);
		}
		return true;
	}
	...
	static bool parse_user_settings(std::istringstream& stream, bool verbose)
	{
		...
		if (!parse_key_value(stream.str(), key, value)) {
			return false;
		}
	...
	{
		std::ifstream file;
		if (!open_file(verbose,
				core::config::game_base_directory / inputs_rel_path, file)) {
			return false;
		}
	...
	bool game_user_settings::load_user_settings(bool verbose)
	{
		...
		if (!open_file(verbose, path, file)) {
			return false;
		}
		...
		for (std::string line; std::getline(file, line);) {
			...
			if (!section_found && !section_start) {
				continue;
			}
