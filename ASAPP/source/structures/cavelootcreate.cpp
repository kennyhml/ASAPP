...
namespace asa::structures
{
	...
	const CaveLootCrate::Quality CaveLootCrate::get_crate_quality()
	{
		if (util::is_only_one_bit_set(quality_flags)) {
			std::cout
				<< "\t[-] Skipped determining drop quality as it is fixed."
				<< std::endl;
		...
		if (!tooltip_area.has_value()) {
			std::cerr << "\t[!] Failed to locate crate tooltip area."
					  << std::endl;
			return ANY;
		}

		window::Color tooltip_white(196, 196, 195);
		auto maskedTooltip = window::get_mask(
			tooltip_area.value(), tooltip_white, 50);
	...
	std::optional<asa::window::Rect> CaveLootCrate::get_info_area()
	{
		auto match_loc = window::locate_template(
			window::Rect(0, 0, 0, 0), resources::text::lootcrate);

		if (!match_loc.has_value()) {
			return std::nullopt;
		}

		return window::Rect(
			match_loc->x - 250, match_loc->y + match_loc->height + 2, 350, 30);
	}
