...
namespace asa::items
{
    ...
    Item::Item(std::string name, json data) : name(name)
    {
        ...
        icon = imread(icon_path, cv::IMREAD_UNCHANGED);
        if (icon.empty()) {
    ...
    {
        if (!inventory_icon.empty()) { return inventory_icon; }

        resize(icon, rgba_inventory_icon,
               cv::Size(icon.cols * 0.24, icon.rows * 0.24), 0, 0,
               cv::INTER_LINEAR);

        cvtColor(rgba_inventory_icon, inventory_icon, cv::COLOR_RGBA2RGB);
        return inventory_icon;
    }

    const cv::Mat& Item::get_inventory_icon_mask()
    {
        if (!inventory_icon_mask.empty()) { return inventory_icon_mask; }

        // Make sure the rgba inventory icon has been created already
        if (rgba_inventory_icon.empty()) { get_inventory_icon(); }
    ...
    {
        if (!notification_icon.empty()) { return notification_icon; }
        resize(icon, rgba_notification_icon,
    ...
    {
        if (!notification_icon_mask.empty()) { return notification_icon_mask; }

        if (rgba_notification_icon.empty()) { get_notification_icon(); }
