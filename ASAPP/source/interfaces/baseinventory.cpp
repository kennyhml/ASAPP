...
namespace asa::interfaces
{
    ...
    window::Rect BaseInventory::get_area() const { return this->area; }
    ...
    void BaseInventory::select_slot(int index) { return select_slot(this->slots[index]); }
                                 ...
                                 bool search)
    {
        ...
        while (auto slot = this->find_item(item, search)) {
            if (i++ > amount && amount != 0) { break; }
            this->take_slot(*slot);
        }
    }

    void BaseInventory::auto_stack() { auto_stack_button.press(); }
}
