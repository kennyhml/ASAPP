...
namespace asa
{
    namespace
    {
                         ...
                         const std::chrono::milliseconds timeout = 500ms)
        {
            ...
            while (!utility::timedout(start, timeout)) {
                cv::Mat img = window::screenshot(icon);
    ...
    bool hud::is_extended_info_toggled() const
    {
        ...
        return window::match(embedded::text::day, roi);
    }
    ...
    {
        return window::match(embedded::text::default_teleport, default_teleport_, 0.5);
    }
    ...
    bool hud::item_added(item& item, cv::Rect* roi_out) const
    {
        ...
        const std::vector<cv::Rect> locations = window::locate_all(
    ...
    bool hud::count_items_added(item& item, int& amount_out) const
    {
        ...
        auto x_loc = window::locate(embedded::text::x, roi);
        if (!x_loc.has_value()) {
        ...
        const cv::Mat img = window::screenshot(roi);
        const cv::Mat mask = utility::mask(img, cv::Vec3b{255, 255, 255}, 50);

        const std::string result_string = window::ocr_threadsafe(
    ...
    bool hud::count_items_removed(item& item, int& amount_out) const
    {
        ...
        cv::Mat img = window::screenshot(roi);
    ...
    void hud::toggle_extended(const bool on, const bool force)
    {
        ...
        if (get_user_setting<bool>("bToggleExtendedHUDInfo")) {
            window::press(keybind);
            return;
        }
        on ? window::down(keybind) : window::up(keybind);
    }

    bool hud::detect_push_notification(const cv::Mat& notification, const float variance)
    {
        if (window::match(notification, push_notifications_, variance)) {
            return true;
        }
        ...
        bool result = utility::await([this, &notification, variance] {
            return window::match(notification, push_notifications_, variance);
        }, 2s);
    ...
    {
        return window::match(embedded::text::removed, area);
    }

    bool hud::item_added(const cv::Rect& area)
    {
        return window::match(embedded::text::added, area);
    }
    ...
    bool hud::mount_hud_available()
    {
        ...
        const cv::Mat mask = utility::mask(window::screenshot(roi), text, 20);
